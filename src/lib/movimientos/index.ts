/**
 * Public API for the Movimientos module.
 *
 * Responsibilities:
 *   - Fetch income_events / essential_expenses / payments for the window
 *     (only this layer touches Supabase).
 *   - Hand them to the pure aggregator.
 *   - Cache in Upstash Redis when configured.
 *
 * Pure logic lives in `buckets.ts`, `aggregator.ts`, `formatters.ts`.
 */
